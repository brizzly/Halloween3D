...
void ClearBound(BOUNDINGBOX *bbox)
{
	...
	bbox->BoxMax.X = -MAX_BOUND;
	bbox->BoxMax.Y = -MAX_BOUND;
	bbox->BoxMax.Z = -MAX_BOUND;
}
...
void CalculateBox(BOUNDINGBOX *Box, pFace Polylist)
{
	...
	for(fListe=Polylist ; fListe ; fListe=fListe->Next)
	{
		for (i=0 ; i<fListe->NumberOfVertices ; i++)
		{
			if(fListe->point[i].X < Box->BoxMin.X) Box->BoxMin.X = fListe->point[i].X;
			if(fListe->point[i].Y < Box->BoxMin.Y) Box->BoxMin.Y = fListe->point[i].Y;
			if(fListe->point[i].Z < Box->BoxMin.Z) Box->BoxMin.Z = fListe->point[i].Z;
			if(fListe->point[i].X > Box->BoxMax.X) Box->BoxMax.X = fListe->point[i].X;
			if(fListe->point[i].Y > Box->BoxMax.Y) Box->BoxMax.Y = fListe->point[i].Y;
			if(fListe->point[i].Z > Box->BoxMax.Z) Box->BoxMax.Z = fListe->point[i].Z; 
		}
	}
}